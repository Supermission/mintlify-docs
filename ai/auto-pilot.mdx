...
The Auto-Pilot system checks for opportunities every 5 minutes:
...
3. **Size the position** — optimal sizing capped by your max-per-trade setting
...
| **Position sizing** | Optimal size capped by max-per-trade |
| **Concurrent positions** | At the position limit? |
| **Correlation check** | Too many bets on the same event or topic? |
| **Confidence threshold** | Meets your minimum confidence? |
| **Edge threshold** | Meets your minimum edge %? |
