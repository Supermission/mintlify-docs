...
<CardGroup cols={2}>
  ...
  <Card title="AI Pipeline" icon="brain">
    **OpenRouter** as primary LLM provider (Llama 3.1, GPT-4O-mini). Vercel AI SDK for streaming and tool calling. 4-agent debate system with judge synthesis. Scheduled analysis runs per market tier.
  </Card>
...
<Steps>
  ...
  <Step title="4 agents analyze independently">
    Bull, Bear, Contrarian, and Quant agents each receive market data + external context (news, social, on-chain). Each produces an independent probability estimate with reasoning.
  </Step>
  ...
  <Step title="Signal emitted if divergence exists">
    If the synthesized probability diverges meaningfully from market price, a signal is stored in `market_ai_analysis` and pushed to the frontend via SSE.
  </Step>
</Steps>
...
| **AI** | Vercel AI SDK 6, OpenRouter, OpenAI SDK, Google GenAI |
